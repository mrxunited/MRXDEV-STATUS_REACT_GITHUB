...
import {
  ...
  DecisionFlow, DecisionFlowStep, ActiveIncidentFlow, ActiveFlowStatus, DecisionFlowStepState 
} from '../../types'; 
...
export const attachMockFlowToIncident = (incidentId: string, flowId: string, flowNameSnapshot: string): ActiveIncidentFlow => {
    ...
    const flowDefinition = mockDecisionFlowsStore.find(f => f.id === flowId); const initialStepStates: Record<string, DecisionFlowStepState> = {};
...
export const updateMockActiveIncidentFlow = (activeFlowId: string, updates: Partial<Omit<ActiveIncidentFlow, 'id'|'$id'|'incidentId'|'flowId'|'flowNameSnapshot'|'startedAt'>>): ActiveIncidentFlow | null => {
    ...
    if (index !== -1) {
        ...
        if (updates.stepStates) { // If stepStates were part of the update, recalculate overall status

...
import { addActivityLog } from './appwrite/activityLogService'; 
...
export function createAdminActivityEmbed(log: UserActivityLog | ActivityLog, siteName: string): DiscordEmbed {
    const isUserActivityLog = 'userId' in log && !('targetType' in log); // Differentiate old UserActivityLog from new ActivityLog
